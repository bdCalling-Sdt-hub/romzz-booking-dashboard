...

    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-4">
          ...
          <IoEyeOutline onClick={() => { handleDetails(record?.id) }} size={22} className="cursor-pointer" />
          {
            record?.status === false ? <MdPerson onClick={() => { handleStatus(record) }} size={25} className="cursor-pointer text-[#00809E]" /> : <MdPersonOff onClick={() => { handleStatus(record) }} size={25} className=" text-red-500 cursor-pointer" />
          }
        </div>
      ),
    },
